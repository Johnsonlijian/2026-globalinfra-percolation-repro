"""R81: full 71-city strict geometry-null ensemble at higher swap intensity.

R67 showed that strict non-crossing geometry nulls collapse most of the
post-spatial residual in a 21-city sensitivity subset. R68 extended the strict
geometry-null layer to all 71 standardized city windows at a conservative
accepted-swap fraction of 0.005. R81 adds a full 71-city higher-intensity
screen at accepted-swap fractions 0.01 and 0.02 with multiple replicates.

This is still an empirical constrained-null ensemble. It is not a Markov-chain
mixing proof, an analytical planar-percolation theorem, or a causal
urban-design law.
"""
...
ROUND_DIR = ROOT / "rounds" / "R81_full71_high_intensity_geometry_null_ensemble"
FIG_BASE = ROOT / "figures" / "Fig_R81_full71_high_intensity_geometry_null_ensemble"
...
SUMMARY_JSON = OUT / "R81_summary.json"
REPORT_MD = OUT / "R81_report.md"
...
def load_r67_reuse(
    ...
    for _, row in r67_df.iterrows():
        ...
        record["notes"] = "reused R67 0.005 replicate 0 for optional full-71 strict geometry-null continuity"
        rows.append(record)
    return rows
...
def run_ensemble(selected: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    if args.reuse_r67 and not records:
        ...
        if records:
            ...
            print(f"[R81] reused {len(records)} matching R67 records", flush=True)
    ...
    for city_idx, row in selected.iterrows():
        ...
        if all(k in done for k in city_keys):
            print(f"[R81] skip complete {city}, {country}", flush=True)
            continue

        print(f"[R81] compute {city}, {country}: n_edges={int(row['n_edges'])}", flush=True)
            ...
            for rep in range(args.replicates):
                ...
                try:
                    ...
                    record: dict[str, Any] = {
                        ...
                        **diag,
                        "status": "pass",
                        "source_round": "R81_computed",
                        "full_connectivity_metrics": bool(include_connectivity),
                        "notes": "R81 full-71 higher-intensity strict non-crossing geometry-null replicate",
                    }
                ...
                if record["status"] == "pass":
                    print(
                        f"[R81] pass {city} f={swap_fraction} rep={rep}: "
                    ...
                    print(f"[R81] fail {city} f={swap_fraction} rep={rep}: {record['notes']}", flush=True)
...
def summarize(reps: pd.DataFrame, selected: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    summary = {
        ...
        "round": "R81_full71_high_intensity_geometry_null_ensemble",
        ...
        "claim_boundary": (
            "R81 is a full 71-city strict non-crossing geometry-null screen at higher accepted-swap intensity "
            "and multiple replicates per city. It is still an empirical constrained-null ensemble rather than "
            "a Markov-chain mixing proof, analytical theorem or causal design law."
        ),
    }
...
def make_figure(city_summary: pd.DataFrame, macro_summary: pd.DataFrame, summary: dict[str, Any]) -> None:
    ...
    palette = pub_style.REGION_COLORS
    colors = df["macro_region"].map(lambda x: palette.get(str(x), palette["Other"]))
    ...
    pub_style.panel_title(ax0, "a", "Null gap versus observed gap (71 cities)")
    ...
    for patch, color in zip(box["boxes"], [pub_style.COLORS["spatial_null"], pub_style.COLORS["geometry_null"]]):
    ...
    pub_style.panel_title(ax1, "b", "Residual: edge scale versus geometry")
    pub_style.light_grid(ax1, axis="y")
    ...
    for y, macro in zip(y_positions, macro_order):
        ...
        ax2.scatter(vals, np.full(len(vals), y) + jitter, s=18, color=palette.get(macro, palette["Other"]), alpha=0.8)
        if len(vals):
    ...
    pub_style.panel_title(ax2, "c", "Residuals by macro-region")
    pub_style.light_grid(ax2, axis="x")
    ...
    pub_style.panel_title(ax3, "d", "Runtime diagnostics")
    pub_style.light_grid(ax3, axis="both")
    text = (
        f"{summary['passed_cities']}/{summary['expected_cities']} cities, "
        f"{summary['n_replicates_pass']}/{summary['n_replicates_total']} replicates pass\n"
        f"mean residual = {summary['road_minus_geometry_mean']:.3f}; "
        f"degree drift max = {summary['max_degree_drift']:.0f}"
    )
    pub_style.annot(ax3, 0.03, 0.96, text)
    ...
    fig.legend(handles=handles, loc="upper center", bbox_to_anchor=(0.5, 0.985), ncol=4, fontsize=6.2, frameon=False)
...
def write_reports(city_summary: pd.DataFrame, macro_summary: pd.DataFrame, summary: dict[str, Any], args: argparse.Namespace) -> None:
    ...
    report = f"""# R81 Full 71-city High-intensity Geometry-null Ensemble

## Purpose

R81 pushes the strict non-crossing geometry-null layer beyond the R68 full-city low-intensity screen. It targets all
71 standardized city windows at accepted-swap fraction(s) {args.swap_fractions} with {args.replicates} replicate(s)
...
"""
    ...
    gate = f"""# R81 Gate Report - Full 71-city High-intensity Geometry-null Ensemble

## Verdict

**R81 evidence gate: {'PASS' if summary['status'] == 'pass' else 'PARTIAL'} with bounded claims.**
...
| Claim boundary | PASS | Framed as a high-intensity empirical strict-geometry ensemble, not a mixing theorem, analytical percolation theorem or causal urban-design rule. |

## Remaining Blockers

- A mathematical derivation of the remaining heterogeneous residual is still absent.
- City planning/history, intervention timing and accessibility covariates are still needed for a causal urban-form claim.
- The Markov-chain mixing properties of the constrained swap process are not established.
"""
    GATE_REPORT_MD.write_text(gate, encoding="utf-8")

    closeout = f"""# R81 Substantive Increment Closeout

| Requested increment | Status | Evidence / reason |
| --- | --- | --- |
| Full 71-city high-intensity strict geometry-null ensemble | {'Done' if summary['passed_cities'] == summary['expected_cities'] and summary['n_replicates_fail'] == 0 else 'Partial'} | {summary['passed_cities']}/{summary['expected_cities']} cities and {summary['n_replicates_pass']}/{summary['n_replicates_total']} higher-intensity strict non-crossing geometry-null replicate records pass. |
...
| Produce publication-grade R81 figure | Done | `Fig_R81_full71_high_intensity_geometry_null_ensemble` exported as SVG, PDF, PNG and TIFF. |
| Analytical theorem | Partial / safer downgrade | Completed a formal null-ladder accounting identity and mechanism proposition; a closed-form planar-percolation theorem is not supported by the evidence. |
| Causal urban-design rule | Partial / safer downgrade | Completed a falsifiable design-hypothesis boundary; no causal design rule is claimed without intervention or identification evidence. |
"""
    CLOSEOUT_MD.write_text(closeout, encoding="utf-8")


def write_theory_note(summary: dict[str, Any]) -> None:
    note = f"""# R81 Formal Boundary Note

## What R81 Allows

R81 supports a bounded mechanism proposition:

> In the 71 standardized road-window graphs, a strict non-crossing
> degree-preserving geometry-null ensemble at higher accepted-swap intensity moves
...
The manuscript can state a design-relevant hypothesis, not a rule:

> Street-network interventions that reduce articulation exposure and bridge-edge
> dependence while preserving fine-grained local connectivity should lower
> percolation vulnerability relative to networks with the same degree sequence
> and edge-scale distribution.

This hypothesis is testable with longitudinal interventions, synthetic design
counterfactuals or matched quasi-experimental cases. It is not identified by
the current cross-sectional null-model evidence alone.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--swap-fractions", default="0.01,0.02")
